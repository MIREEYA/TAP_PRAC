...
//CAMBIOS YAGS
    ...
    if(yags_nelt != 4)
      fatal("bad yags config (1 <x> <g>) 0");
    if(btb_nelt != 2)
      fatal("bad btb config (<num_sets> <associativity>)");

    pred = bpred_create(BPredYags,
			  /* bimod table size */0, 
			  /* l1 size */yags_config[0],
			  /* l2 size */yags_config[1],
			  /* meta table size */yags_config[0],
